...
DUMPPREAMBLE=: 0 : 0
NB.
NB. This dump script can be directly loaded by J with
NB. standard 0!:0 script commands if the following
NB. conditions are satisfied:
NB.
NB.   1) J has been loaded with the standard profile
NB.   2) JOD has been loaded
NB.   3) A target JOD put dictionary is open read/write
NB.
NB. Note: JOD rebuild commands are marked with the
NB. comment NB.{*JOD*} so they can be easily found
NB. with with text editors.
NB.
NB. Dump scripts can be loaded with expressions like:
NB.
NB.   0!:0 <'c:\temp\dumpscript.ijs'
)
...
NB.*wttext v-- Returns a character list of annotated word or test
NB. text.  (y)  is  boxed a  (name,class,value)  or (name,value)
NB. table
NB.
NB. dyad:  iaObject wttext bt

'obj noex'=. x  NB. object code and explanation bit
